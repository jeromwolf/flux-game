...
6. **지뢰찾기** (Puzzle) - 논리 퍼즐, 3가지 난이도
7. **워드 타워** (Puzzle) - 단어 쌓기, 어휘력 향상
8. **Merge Master** (Puzzle) - 육각형 그리드 숫자 병합 **(NEW!)**
9. **스네이크** (Action) - 뱀 게임, 터치 컨트롤
10. **브레이크아웃** (Action) - 블록 깨기, 파워업
11. **다이노 런** (Action) - 공룡 러너, 파워업 시스템
12. **Seoul Runner** (Action) - 서울 배경 엔들리스 러너
13. **Liquid Robot** (Action) - 변신 로봇 미션 게임
14. **Space Shooter** (Action) - 우주 슈팅, 보스전
15. **버블 슈터** (Arcade) - 색깔 매칭, 콤보 시스템
16. **플래피 플럭스** (Arcade) - 파이프 피하기, 중력 물리
17. **Stack Tower** (Arcade) - 블록 쌓기 타이밍 게임
18. **Cube Collector 3D** (Arcade) - 3D 큐브 수집
19. **틱택토** (Strategy) - AI 대전, 미니맥스 알고리즘
20. **Island Survival** (Strategy) - 무인도 생존 시뮬레이션
